...
exports.createEmbeddingsWithChunks = createEmbeddingsWithChunks;
exports.storeDocument = storeDocument;
exports.storeDocumentWithChunks = storeDocumentWithChunks;
...
// トークンエンコーダー
const enc = (0, tiktoken_1.encoding_for_model)("text-embedding-3-small");
// チャンク化設定
const CHUNK_CONFIG = {
...
// テキストをトークン数でチャンク化
function chunkTextByTokens(text, maxTokens = CHUNK_CONFIG.maxTokens, overlap = CHUNK_CONFIG.overlap) {
    const chunks = [];
...
// チャンク化された埋め込み生成
async function createEmbeddingsWithChunks(text, maxTokens = CHUNK_CONFIG.maxTokens) {
...
// チャンク化されたドキュメント保存
async function storeDocumentWithChunks(document) {
